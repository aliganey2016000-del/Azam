...
async function main() {
  ...
  async function user(email: string, roleId: string) {
    const record = await prisma.user.create({ data: { email, passwordHash, status: UserStatus.ACTIVE, roles: { create: { roleId } } } });
    return record;
  }
  ...
  console.log(`Seeded demo data for ${city.name}, ${department.name}.`);
}
