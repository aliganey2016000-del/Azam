...
model Application { id String @id @default(uuid()) @db.Uuid; applicationNumber String @unique; studentId String @db.Uuid; universityId String? @db.Uuid; programmeId String? @db.Uuid; specialtyId String? @db.Uuid; source StudentSource; status ApplicationStatus @default(DRAFT); rejectedReason String?; rejectedAt DateTime?; rejectedById String? @db.Uuid; student Student @relation(fields: [studentId], references: [id]); university University? @relation(fields: [universityId], references: [id]); programme Programme? @relation(fields: [programmeId], references: [id]); specialty Specialty? @relation(fields: [specialtyId], references: [id]); documents ApplicationDocument[]; history ApplicationStatusHistory[]; placement Placement?; createdBy User? @relation("ApplicationCreatedBy", fields: [createdById], references: [id]); createdById String? @db.Uuid; createdAt DateTime @default(now()); updatedAt DateTime @updatedAt; @@index([status, createdAt]); @@index([studentId]) }
